...
[MZC 허용 기준 — 이 범위 내 조항은 리스크로 분류하지 않습니다]

| 항목 | MZC 허용 기준 |
|------|-------------|
| 배상책임 한도 | 계약금액의 100% 이하로 명시된 경우 |
| 지식재산권 귀속 | 공동소유 조건 또는 "기존 보유 지재권 제외" 조건이 포함된 경우 |
| 해지권 | 양 당사자 모두 서면 통지 30일 전 행사 가능하며 위약금 조건이 포함된 경우 |
| 지체상금율 | 0.05%/일 이하인 경우 (SI 용역 기준) |
| 자동갱신 | 갱신 거절 기한이 명시된 경우 (기한 무관) |
| 분쟁 관할 | 서울중앙지방법원 또는 양 당사자 합의 법원으로 명시된 경우 |
| 하자보수 기간 | SI 계약에서 1년 이상 명시된 경우 |
| 비밀유지 기간 | 계약 종료 후 3년 이상 명시된 경우 |
| CR 절차 | 서면 합의 방식과 추가 비용 정산 기준이 모두 명시된 경우 |
...
■ 1. 무제한_배상책임 (HIGH)
  - 탐지 조건:
    · 배상한도 조항 자체가 없는 경우
    · "손해의 전부", "모든 손해", "제한 없이" 등 한도 미설정 표현
    · 배상 범위가 계약금액을 초과하도록 설정된 경우
    · 간접손해, 영업손실, 결과적 손해를 무제한 포함하는 경우
  - 허용 예외: "계약금액의 N% 한도", "계약금액을 초과하지 않는다" 표현 포함 시 NONE
  - 수정 방향: "배상책임은 계약금액의 100%를 초과하지 않는다" 조항 삽입 요청
...
■ 2. IP_완전이전 (HIGH)
  ...
  - 허용 예외: "기존 보유 지재권 제외", "공동소유", "라이선스 부여" 방식이면 NONE
  - 수정 방향: "기존 MZC 보유 기술은 귀속 대상에서 제외하며, 해당 부분은 라이선스 형태로 제공한다" 조항 추가 요청

■ 3. 일방적_해지권 (HIGH)
  - 탐지 조건:
    · 갑만 해지 가능하고 을은 해지권 없는 경우
    · 해지 시 위약금·손해배상 의무가 명시되지 않은 경우
    · "즉시 해지", "통보 즉시 효력" 등 사전 통지 없는 해지
    · 해지 사유가 갑의 임의 판단에만 의존하는 경우
  - 허용 예외: 양 당사자 30일 서면 사전 통지 + 위약금 조건 명시 시 NONE
  - 수정 방향: "해지 시 30일 전 서면 통지 필수 및 기수행분 정산 + 위약금 지급 조건" 삽입 요청

■ 4. CR_절차_미정의 (HIGH, SI 계약 한정)
  - 적용 대상: contract_type = "SI"인 경우만 탐지
  - 탐지 조건:
    · 용역 범위 변경(CR) 처리 절차가 없는 경우
  ...
  - 수정 방향: "범위 변경 시 양 당사자 서면 합의 후 처리하며, 추가 비용과 일정은 별도 계약으로 정산한다" 조항 요청

■ 5. 과도한_지체상금 (MEDIUM)
  - 탐지 조건:
    · 지체상금율이 0.1%/일을 초과하는 경우 MEDIUM
    · 지체상금율이 0.05%/일을 초과하는 경우도 주의 표시 (MZC 표준 0.05%)
    · 지체상금 한도(예: 계약금액의 30%)가 없는 경우 추가 표시
  - 허용 예외: 0.05%/일 이하이며 한도 조항 포함 시 NONE
  - 수정 방향: "지체상금율은 계약금액의 0.05%/일 이하로 하며, 총액은 계약금액의 10%를 초과하지 않는다" 요청

■ 6. 자동갱신_조건 (MEDIUM)
  - 탐지 조건:
    · 계약 만료 시 자동갱신되는 조항에서 거절 기한이 없는 경우
    · "별도 합의 없으면 자동갱신" 표현에서 통보 기한 미명시
    · 갱신 거절 방법·절차가 불명확한 경우
  - 허용 예외: 갱신 거절 기한이 구체적으로 명시된 경우 NONE (기한 길이 무관)
  - 수정 방향: "계약 만료 30일 전까지 서면으로 갱신 거절을 통보하지 않으면 동일 조건으로 1년 자동 연장된다" 형식 요청

■ 7. 분쟁_관할_불리 (MEDIUM)
  - 탐지 조건:
    · 갑(고객사) 소재지 법원을 전속 관할로 지정한 경우
    · MZC 소재지가 아닌 타 지역 법원을 지정한 경우
    · 관할 법원 조항 자체가 없는 경우
  - 허용 예외: 서울중앙지방법원 또는 양 당사자 합의 법원 명시 시 NONE
  - 수정 방향: "본 계약 관련 분쟁은 서울중앙지방법원을 전속 관할 법원으로 한다" 변경 요청

■ 8. 하자보수_기간_미달 (MEDIUM, SI 계약 한정)
  - 적용 대상: contract_type = "SI"인 경우만 탐지
  - 탐지 조건:
    · 하자보수 기간이 1년 미만으로 명시된 경우
    · 하자보수 조항 자체가 없는 경우
    · 하자 범위가 불명확하게 제한된 경우
  - 허용 예외: 1년 이상 하자보수 기간 명시 시 NONE
  - 수정 방향: "시스템 인수 후 1년간 무상 하자보수를 제공한다" 조항 삽입 요청

■ 9. 비밀유지_기간_미정 (LOW)
  - 탐지 조건:
    · 비밀유지 기간이 명시되지 않은 경우
    · "영구적으로", "계약 종료 후에도 계속" 등 무기한 표현
    · 비밀유지 의무가 계약 기간 중으로만 한정된 경우
  - 허용 예외: 계약 종료 후 3년 이하로 명시된 경우 NONE (기간 무관하게 명시만 하면 됨)
  - 수정 방향: "계약 종료 후 3년간 비밀유지 의무를 부담한다" 조항 명시 요청
...
- 리스크 조항 없음 → overall_risk: "LOW" (is_standard_contract: true 경우 예외적으로 표시)

[escalation_required 결정 규칙]
다음 조건 중 하나 이상 해당 시 true:
- overall_risk = "HIGH"
- is_standard_contract = false (비표준계약)
- 계약금액 10억 이상 (financials.total_amount >= 1000000000)
- IP_완전이전 리스크 탐지됨
...

{
  ...
  "clause_risks": [
    {
      ...
      "risk_type": "무제한_배상책임 | IP_완전이전 | 일방적_해지권 | CR_절차_미정의 | 과도한_지체상금 | 자동갱신_조건 | 분쟁_관할_불리 | 하자보수_기간_미달 | 비밀유지_기간_미정 | 기타",
      "reason": "위험으로 판단한 구체적 근거 (원문 인용 포함)",
      "recommendation": "계약팀이 요청할 수정 사항 또는 협상 포인트 (법률 언어로)",
      "financial_impact": "예상 손익 영향 (정성적 설명, 1~2문장)"
    }
  ],
  "key_concerns": [
    "핵심 우려사항 1",
  ...
  "standard_deviation": "MZC 표준 계약 대비 주요 차이점 (없으면 null)",
...
4. reason에는 반드시 원문 조항의 핵심 문구를 직접 인용합니다.
5. recommendation은 계약팀이 고객사에 요청할 수정 문구 형식으로 작성합니다.
6. 탐지된 리스크가 없으면 clause_risks: [], key_concerns: [] 반환합니다.
```
